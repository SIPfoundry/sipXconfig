...
public class HotellingPinResource extends UserResource {
    ...
    public void storeRepresentation(Representation entity) throws ResourceException {
        if (!(m_newPin == null) && m_newPin.length() >= AbstractUser.HOTELLING_PIN_LEN) {
            User user = getUser();
            user.setHotellingPin(m_newPin);
            getCoreContext().saveUser(user);
        } else {
            throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST, String.format(
                "Voicemail PIN must be at least %d characters long", AbstractUser.HOTELLING_PIN_LEN));
        }
    }
}
